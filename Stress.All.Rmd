...
```{r}
#Make the nested anova
nest_s <- aov(stress.clim.ca.3$YII.var ~ stress.clim.ca.3$Clim3 / factor(stress.clim.ca.3$PopID) / factor(stress.clim.ca.3$Individual))
summary(nest_s)
...
Creating the multiple regression model
```{r}
smod <- lm(YII ~ tmean + Elevation + tmax + tdmean + vpdmax + PPT + vpdmin + solclear, data=stress.clim.ca.2)
...
summary(pmod) #adjusted r squared = 0.0886, p-value = 0.0238
Anova(pmod, type=2)
par(mfrow = c(2, 2))
plot(pmod)
par(mfrow = c(1, 1))
...
#Enter number of PCs to retain
#Enter number of groups to retain
climgroups$Kstat
climgroups$stat
climgroups$size
...
```{r}
datp <- dat1[!duplicated(dat1$PopID),]
rownames(datp) = datp$PopID
datp <- datp[,-c(1)]
head(datp)
datp = data.frame(scale(datp))
euclid_pops=dist(datp, method ="euclidean")
tree2 = hclust(euclid_pops, method="ward.D")
colors = c("#00AFBB", "#E7B800", "#FC4E07")
clus3 = cutree(tree2,3)
plot(as.phylo(tree2), main="Boechera Climate Regions", type = "tidy", cex=1.4, 
     tip.color=colors[clus3], label.offset=0.2) 
#library(ggraph)
#ggraph(tree2, layout = 'auto', circular = FALSE) +  eom_edge_diagonal() +
  #geom_node_point() + theme_void()
clus2 = cutree(tree2,2)
plot(as.phylo(tree2), main="Boechera Climate Regions", type = "tidy", cex=1.4, 
     tip.color=colors[clus2], label.offset=0.2) 
```
...
clus3 = cutree(tree,3)
plot(as.phylo(tree), main="Boechera Climate Regions", type="fan", cex=1, 
     tip.color=colors[clus3], label.offset=0.2) 
library(ggdendro)
p <- ggdendrogram(tree, rotate=TRUE, theme_dendro=TRUE)
library(pvclust)
tree=pvclust(dat2, method.hclust="ward.D", nboot=1000,method.dist="euclidean")
# Create the dendrogram with p values
plot(tree, hang=-1, main="Boechera Climate Regions")
# add rectangles around g

AIC(tree)
```

```{r}
nmds_out=nmds(euclid, mindim=2, maxdim=2) # runs NMDS
scores=nmds.min(nmds_out) # generate scores
nmds_out$stress
PopID = dat1[,1]
mygroups = sort(unique(PopID)); mygroups 
plot(scores, pch=19, col=mycol[as.factor(PopID)])
...
library(vegan)
ordiellipse(scores, PopID, conf=0.6, col=mycol, show.groups=mygroups) 
```


```{r}
kmeans3=kmeans(euclid, centers=3, iter.max=50, nstart=25)
kmeans3$size; kmeans3$centers; kmeans3$cluster
out1=metaMDS(euclid, k=2)
...
Loading in Bruvo Distance Matrix
```{r}
...
anova(a4)
AIC(a)
AIC(a2) #this is lower than aic(a). This model has a better score
AIC(a3)
AIC(a4)
BIC(a);BIC(a2)
library(modEvA)
...
```{r}
Dsquared(m1,adjust=T);Dsquared(m2,adjust=T)
```
...
sum.pmod
plot(sum.pmod$adjr2,xlab = "Number of Variables", ylab = "Adjusted RSq")
adj_r2_max = which.max(sum.pmod$adjr2) #8
points(adj_r2_max, sum.pmod$adjr2[adj_r2_max], col ="red", cex = 2, pch = 20)
...
sum.pmod
plot(sum.pmod$bic,xlab = "Number of Variables", ylab = "BIC")
bic_max = which.max(sum.pmod$bic) #4
points(bic_max, sum.pmod$bic[bic_max], col ="red", cex = 2, pch = 20)
```

```{r}
m1 <- lm(YII ~ tmean + Elevation + tmax + tdmean + tmin + PPT + vpdmin + solclear, data=pg)
summary(m1)
m2 <- glm(YII ~ MultlocFis + Eff_num + Hs + Gis, data=pg)
Dsquared(m1,adjust=T);Dsquared(m2,adjust=T)
m3 <- glm(YII~  MultlocFis, data=pg)
summary(m3)
Dsquared(m3)
```
...
Creating a YII distance matrix
```{r}
...
Creating a gentetic matrix of the same size
```{r}
...
```{r}
dist.df2 <- data.frame( NeiG=ngpop2[lower.tri(ngpop2)], ClimDist=euclid.clim.subset[ lower.tri(euclid.clim.subset)], GeogDist = km.subset.d[lower.tri(km.subset.d)], Stress = Y2d.stress[lower.tri(Y2d.stress)])
```
...
M1c <-glm(Stress ~ NeiG, data=dist.df2)
M2a <-glm(Stress ~ NeiG + ClimDist, data=dist.df2)
M2b <-glm(Stress ~ NeiG + GeogDist, data=dist.df2)
M2c <-glm(Stress ~ ClimDist + GeogDist, data=dist.df2)
M3 <- glm(Stress ~ ClimDist + GeogDist + NeiG, data=dist.df2)
