...
To add in climatic clusters, run this chunk
```{r}
...
stress.clim.ca.2[19:28] <- scale(stress.clim.ca.2[19:28])
```


### 5 DAY RECOVERY CHLOROPHYLL FLUORESCENCE DATA ###

Recovery YII averaged for each subpopulation. Graph.
```{r}
...
Recovery YII averaged for each species. Graph.
```{r}
...
#To look at different climate groups
...
Plotting YII against individual climatic variables (Using LM best fit line)
```{r}
...
Plotting YII against climatic variables (Using Loess Best Fit)
```{r}
p1 <- ggplot(data=recovery.clim.ca, aes(x=tmean, y=YII)) +
  geom_point() + theme_bw() + xlab("Mean Annual Temperature") +
...
Initial YII averaged for each subpopopulation
```{r}
...
Ion Leakage plot based on subpopulation
```{r}
...
Ion Leakage plot based on species
```{r}
...
Testing it out, one variable at a time
```{r}
...
hist(mahal_sq)
mahal=sqrt(mahal_sq)
#mahal # check how the distance matrix looks like
```
...
Loading in Population Distance Matrix (meters)
```{r}
...
Making multiple regressions using distance matrices based on climate and physical distance to predict genetic distance. Climate varaibles are also tried individually. 
```{r}
...
##Creating a distance matrix to reflect stress response across subppulations ##
...
Y2b <- aggregate(YII ~ PopID, 
                    data=Y2, 
                    function(x) { 
                      c(avg=var(x))  #Set to 'var' or 'mean', depending which you want
                    })
Y2b
...
Distance matrix based on ion leakage
```{r}
...
Distance matrix based on chlorosis score
```{r}
...
##Creating Genetic distance matrix based on different metrics. Choose one chunk to run. ##
...
Sqrt transforming everything (unneccesary, just checking)
```{r}
...
log transforming everything (unneccesary, just checking)
```{r}
gendis <- log(gendist)
...
head(climdist)
head(km.subset.d)
head(Y2d.stress) #scale if necessary
```


Multiple regression on distance matrices. Trying many combinations
```{r}
library(ecodist)
...
More MRM combinations, based on individual climate factor
```{r}
head(dat3)
