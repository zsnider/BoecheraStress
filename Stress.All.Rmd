...
To scale the climate data, run this chunk
```{r}
recovery.clim.ca.2[19:28] <- scale(recovery.clim.ca.2[19:28])
stress.clim.ca.2[19:28] <-
```
...
```{r}
recoveryplot2 <- ggplot(data=recovery.clim.ca.2,aes(x=as.factor(Species), y=as.numeric(YII), fill=as.factor(Species))) +
  geom_boxplot() +
  theme_bw(base_size=20) +
  ylab("YII") +
  xlab("Boechera Species") +
...
YII variance plot
```{r}
vplot <- ggplot(data=recovery.clim.ca.3, aes(x=as.factor(PopID), y=as.numeric(YII.mean),
                                           fill=as.factor(Clim3))) +
  geom_boxplot() +
  theme_bw() +
...
```{r}
rmod.fit <- regsubsets(YII ~ tmean + Elevation + tmax + tdmean + vpdmax + tmin + PPT + vpdmin + solclear + soltrans, data=recovery.clim.ca.2, nvmax=10)
...
```{r}
rmod <- lm(YII ~ tmean + Elevation + tmax  + tdmean + vpdmax + tmin + PPT + vpdmin + solclear + soltrans, data=recovery.clim.ca.2)
...
p1 <- ggplot(data=recovery.clim.ca, aes(x=tmean, y=YII)) +
  ...
  geom_smooth(method="loess", alpha=0.3, linetype="longdash", color="#0072B2")
...
ANOVA for diploids vs polyploids
```{r}
nest_species <- aov(stress.clim.ca.3$YII.mean ~ stress.clim.ca.3$Species)
summary(nest_species)
m <- lmer(YII ~ Species + (1|PopID), data=stress.clim.ca.2)
summary(m)
```
...
adj_r2_max = which.max(sum.smod$adjr2) #9
...
To scale the climate data, run this chunk
```{r}
ion.clim.ca <- ion.clim.ca[,-c(13,21)]
ion.clim.ca[12:21] <- scale(ion.clim.ca[12:21])
```

Ion Leakage plot
```{r}
ioncaplot <- ggplot(ion.clim.ca, aes(x=Species, y=Percent.Leaked, fill=Species)) +
  geom_boxplot() +
...
```{r}
#Make the nested anova
nest_i <- aov(ion.clim.ca$Percent.Leaked ~factor(ion.clim.ca$Region) / factor(ion.clim.ca$PopID))
summary(nest_i)
TukeyHSD(nest_i)$'factor(ion.clim.ca$Region)'
```
...
To scale climate data, run this chunk
```{r}
pheno.clim.ca <- pheno.clim.ca[,-c(11,19)]
pheno.clim.ca[10:19] <- scale(pheno.clim.ca[10:19])
```
...
```{r}
elev=dist(dat2[1], method ="euclidean")
tmean=dist(dat2[2], method ="euclidean")
tmax=dist(dat2[3], method ="euclidean")
tmin=dist(dat2[4], method ="euclidean")
ppt=dist(dat2[5], method ="euclidean")
tdmean=dist(dat2[6], method ="euclidean")
vpdmax=dist(dat2[7], method ="euclidean")
vpdmin=dist(dat2[8], method ="euclidean")
solrad=dist(dat2[9], method ="euclidean")
cloudtrans=dist(dat2[10], method ="euclidean")
```
...
```{r}
MRM(bruv2 ~  euclid + km2, nperm=1000, method="linear", mrank=F)
MRM(bruv2 ~  euclid, nperm=1000, method="linear", mrank=F)
MRM(bruv2 ~  km2, nperm=1000, method="linear", mrank=F)
MRM(bruv2 ~ km2 + elev + euclid, method="linear", mrank=F)
MRM(bruv2 ~ km2 + elev + tmean + tmax + tmin + ppt + tdmean + vpdmax + vpdmin + solrad + cloudtrans, method="linear", mrank=F)
```
...
```{r}
chlorosis <- data.frame(pheno.clim.ca$Individual, pheno.clim.ca$PopID, pheno.clim.ca$Score.mean, pheno.clim.ca$Score.var)
names(chlorosis) <- c("Individual", "PopID","Score.mean","Score.var")
chlorosis2 <- aggregate(Score.mean ~ PopID, 
                    data=chlorosis, 
                    function(x) { 
                      c(avg=mean(x)) 
                    })
chlorosis2
chlorosis2 <- chlorosis2[-3,]
chlorosis.dist <- dist(chlorosis2, method="euclidean")
```
...
```{r}
ngpop <- read.csv("NeiGpopdist3.csv")
rownames(ngpop) <- ngpop$X
ngpop <- ngpop[,-1]
gendist <- as.dist(ngpop)
```
...
```{r}
ngpop <- read.csv("geneticjostdpop2.csv")
rownames(ngpop) <- ngpop$X
ngpop <- ngpop[,-1]
gendist <- as.dist(ngpop)
```
...
dat3 <- dat3[-3,]
head(dat3)
#dat3 = data.frame(scale(dat3))
#hist(dat3)
climdist=dist((data.frame(scale(dat3))), method ="euclidean")
...
mahal=sqrt(mahal_sq)
dat4 = decostand(dat3, method="standardize")
head(dat4)
climdist = dist(dat4, method="euclidean")
?dist
```
...
```{r}
Y2d.stress <- scale(Y2d.stress)
```

Multiple regression on distance matrices
```{r}
library(ecodist)
MRM(Y2d.stress ~ gendist + mahal_sq + km.subset.d, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ gendist + climdist + km.subset.d, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ gendist, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ climdist + km.subset.d, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ km.subset.d, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ mahal_sq, nperm=1000, method="linear", mrank=F)
MRM(gendist ~ km.subset.d + climdist, nperm=1000, method="linear",mrank=F)

AIC(MRM1)
MRM(Ion.dist ~ gendist + mahal_sq + km.subset.d, nperm=1000, method="linear", mrank=F)
MRM(Ion.dist ~ gendist + climdist + km.subset.d, nperm=1000, method="linear", mrank=F)
MRM(Ion.dist ~ km.subset.d + climdist, nperm=1000, method="linear", mrank=FALSE)
MRM(Ion.dist ~ km.subset.d, nperm=1000, method="linear", mrank=F)
MRM(Ion.dist ~ climdist, nperm=1000, method="linear", mrank=FALSE)
MRM(chlorosis.dist ~ gendist + climdist + km.subset.d, nperm=1000, method="linear", mrank=F)
MRM(chlorosis.dist ~ climdist + km.subset.d, nperm=1000, method="linear", mrank=F)
```
...
distance matrix for each climate variable based on euclidean distances
```{r}
head(dat3)
#dat3 <- decostand(dat3, method="hellinger")
elev=dist(dat3[1], method ="euclidean")
tmean=dist(dat3[2], method ="euclidean")
tmax=dist(dat3[3], method ="euclidean")
tmin=dist(dat3[4], method ="euclidean")
ppt=dist(dat3[5], method ="euclidean")
tdmean=dist(dat3[6], method ="euclidean")
vpdmax=dist(dat3[7], method ="euclidean")
vpdmin=dist(dat3[8], method ="euclidean")
solrad=dist(dat3[9], method ="euclidean")
cloudtrans=dist(dat3[10], method ="euclidean")
```
each climate matrix based on mahal
```{r}
elev=distance(dat3[1], method ="mahal")
tmean=distance(dat3[2], method ="mahal")
tmax=distance(dat3[3], method ="mahal")
tmin=distance(dat3[4], method ="mahal")
ppt=distance(dat3[5], method ="mahal")
tdmean=distance(dat3[6], method ="mahal")
vpdmax=distance(dat3[7], method ="mahal")
vpdmin=distance(dat3[8], method ="mahal")
solrad=distance(dat3[9], method ="mahal")
cloudtrans=distance(dat3[10], method ="mahal")
```

```{r}
MRM(Y2d.stress ~ gendist + km.subset.d + elev + tmean + tmax + tmin + ppt + vpdmax + vpdmin + solrad + cloudtrans, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ gendist, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ km.subset.d, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ elev, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ tmean, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ tmax, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ tmin, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ ppt, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ vpdmax, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ vpdmin, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ solrad, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ cloudtrans, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ gendist + km.subset.d + elev + tmean + tmin + ppt, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ gendist + km.subset.d + elev + tmean + tmax + tmin + tdmean + vpdmax + solrad + cloudtrans, nperm=1000, method="linear", mrank=F)
MRM(Ion.dist ~ gendist + km.subset.d + elev + tmean + tmax + tmin + ppt + tdmean + vpdmax + vpdmin + solrad + cloudtrans, nperm=1000, method="linear", mrank=F)
MRM(Y2d.stress ~ gendist + km.subset.d + elev + tmean, nperm=1000, method="linear", mrank=F)
```


```{r}
head(dat3)
temp=dist(dat3[,c(2,3,4,6)], method ="euclidean")
vpd=dist(dat3[,c(7,8)], method="euclidean")
MRM(Y2d.stress ~ gendist + km.subset.d + elev + temp + ppt + vpd + solrad + cloudtrans, nperm=1000, method="linear", mrank=F)
```
...
```{r}
dist.df <- data.frame( GenDist=gendist[lower.tri(gendist)], ClimDist=climdist[lower.tri(climdist)], GeogDist = km.subset.d[lower.tri(km.subset.d)])
summary(dist.df)
distplot1 <- ggplot(dist.df,aes(x=ClimDist,y=GenDist)) + 
  geom_point() +
  theme_bw(base_size=20) +
  xlab("Climate Distance") + ylab("Genetic Distance (linearized D)") +
  geom_smooth(method="lm", alpha=0.15, linetype="solid", color="#0072B2")
distplot1
distplot2 <- ggplot(dist.df,aes(x=GeogDist,y=GenDist)) + 
  geom_point() +
  theme_bw(base_size=20) +
  xlab("Geographic Distance (kilometers)") + ylab("Genetic Distance (linearized D)") +
  geom_smooth(method="lm", alpha=0.15, linetype="solid", color="#0072B2")
distplot2
```
...
```{r}
dist.df <- data.frame( GenDist=gendist[lower.tri(gendist)], ClimDist=climdist[ lower.tri(climdist)], GeogDist = km.subset.d[lower.tri(km.subset.d)], Stress = Y2d.stress[lower.tri(Y2d.stress)])
mrm1 <- lm(Stress~GenDist + ClimDist + GeogDist, data=dist.df)
#mrm1 <- lm(dist(Stress)~dist(GenDist) + dist(ClimDist) + dist(GeogDist), data=dist.df)
AICc(mrm1)
dist.df3 <- data.frame( GenDist=gendist[lower.tri(gendist)], ClimDist=climdist[ lower.tri(climdist)], GeogDist = km.subset.d[lower.tri(km.subset.d)], Stress = Y2d.stress[lower.tri(Y2d.stress)], elev=elev[lower.tri(elev)], tmean=tmean[lower.tri(tmean)], tmax=tmax[lower.tri(tmax)],tmin=tmin[lower.tri(tmin)],ppt=ppt[lower.tri(ppt)],tdmean=tdmean[lower.tri(tdmean)],vpdmax=vpdmax[lower.tri(vpdmax)],vpdmin=vpdmin[lower.tri(vpdmin)],solrad=solrad[lower.tri(solrad)],cloudtrans=cloudtrans[lower.tri(cloudtrans)])
mrm2 <- lm(Stress ~ GenDist + GeogDist + elev + tmean + tmax + tmin + ppt + tdmean + vpdmax + vpdmin + solrad + cloudtrans, data = dist.df3)
AICc(mrm2)
```
